...
def ensemble_align(depth: np.ndarray,
    scale_invariant: bool = True,
    shift_invariant: bool = True,
    output_uncertainty: bool = False,
    reduction: str = "median",
    ...
    depth_torch = torch.from_numpy(depth).to(torch.float64) if isinstance(depth, np.ndarray) else depth
    if depth_torch.dim() == 3:
        depth_torch = depth_torch.unsqueeze(1)  # (B,1,H,W)
    
    if depth_torch.dim() != 4 or depth_torch.shape[1] != 1:
        raise ValueError(f"Expecting 4D array/tensor of shape [B,1,H,W]; got {depth_torch.shape}.")
    if reduction not in ("mean", "median"):
        raise ValueError(f"Unrecognized reduction method: {reduction}.")
    if not scale_invariant and shift_invariant:
        raise ValueError("Pure shift-invariant ensembling is not supported.")
    ...
    def compute_param(depth: torch.Tensor):
        ...
        depth_to_align = depth.to(torch.float64)
        if max_res is not None and max(depth_to_align.shape[2:]) > max_res:
            depth_to_align = resize_max_res(
                depth_to_align, max_res, get_tv_resample_method("nearest-exact")
            )
    ...
    requires_aligning = scale_invariant or shift_invariant
    ensemble_size = depth_torch.shape[0]

    if requires_aligning:
        param = compute_param(depth_torch)
        depth_torch = align(depth_torch, param)

    # depth, uncertainty = ensemble(depth, return_uncertainty=output_uncertainty)
    depth_max = depth_torch.max()
    if scale_invariant and shift_invariant:
        depth_min = depth_torch.min()
    elif scale_invariant:
        depth_min = 0
    else:
        raise ValueError("Unrecognized alignment.")
    depth_range = (depth_max - depth_min).clamp(min=1e-6)
    depth_torch = (depth_torch - depth_min) / depth_range

    # Convert back to np.ndarray before returning
    return depth_torch.cpu().numpy()